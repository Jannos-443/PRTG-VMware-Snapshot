...
try {
    ...
    Write-Output "<prtg>"
    Write-Output " <error>1</error>"
    Write-Output " <text>Error Loading VMware Powershell Module `"VMware.VimAutomation.Core`" ($($_.Exception.Message))</text>"
    Write-Output "</prtg>"
    Exit
}
...
# Connect to vCenter
$null = Connect-VIServer -Server $ViServer -Credential $Credentials
